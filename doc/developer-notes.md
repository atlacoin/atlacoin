...
- [Development guidelines](#development-guidelines)
    - [General Atlacoin](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Atlacoin uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Atlacoin is a multi-threaded application, and deadlocks or other
...
Atlacoin can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Atlacoin, where everyone uses
...
pay attention to for reviewers of Atlacoin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Atlacoin uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Atlacoin, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Atlacoin so that they can be integrated
...
file descriptors, it will cause problems with Atlacoin's `select()` loop, because
...
compatibility between nodes. This happened in Atlacoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Atlacoin's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Atlacoin, the wallet was always
